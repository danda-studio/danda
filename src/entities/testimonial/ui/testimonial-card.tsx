...
export function TestimonialCard({ testimonial }: { testimonial: Testimonial }) {
  ...
  const fontClassName = isLight ? "font-[family-name:var(--font-inter-sans)]" : "font-[family-name:var(--font-manrope-sans)]";

  return (
    <div
      className={cn(
        "flex h-[14rem] w-[23.25rem] shrink-0 flex-col items-start justify-center rounded-[1.5rem] p-[1.5rem]",
        isLight ? "" : "bg-[var(--color-gray-850)]",
      )}
      ...
      <div className="flex w-full flex-1 flex-col items-start justify-between">
        <p
          className={cn(
            ...
            fontClassName,
            isLight ? "text-[var(--color-gray-800)]" : "text-white",
          )}
        >
          {testimonial.quote}
        </p>
        <div className="flex w-full items-center gap-[1rem]">
          <img alt="" src={testimonial.avatar} className="size-[3.5rem] shrink-0 rounded-full object-cover" />
          <div className="flex flex-1 flex-col items-start justify-center gap-[0.125rem]">
            <p className={cn("text-[1.25rem] tracking-[-0.0625rem] whitespace-nowrap", fontClassName, isLight ? "text-[var(--color-gray-800)]" : "text-white")}>
              {testimonial.name}
            </p>
            <p className={cn("text-[1rem]", fontClassName, isLight ? "text-[var(--color-gray-350)]" : "text-[var(--color-gray-400)]")}>
              {testimonial.role}
            </p>
          </div>
        </div>
